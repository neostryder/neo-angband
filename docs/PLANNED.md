...
`display_message`'s inner loop (`ui-input.c` L546-586) splits a message that is
longer than the line at the rightmost space, prints the first part, prompts, and
recurses on the rest; `msg_flush_split_existing` (L407-446) does the same to a
line that has already been written. This port's pager works in whole messages:
one longer than the line becomes a page of its own and the renderer truncates
it. Nothing is lost from the recall screen, so this is what a player sees rather
than what the game recorded, but a very long message does read differently here.
Closing it means giving `packMessages` a within-message split and a cursor that
can name a point inside a message rather than only a message boundary.
