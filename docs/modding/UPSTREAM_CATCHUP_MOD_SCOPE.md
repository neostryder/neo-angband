...
**#118 is built**, behind the engine's new `shapeLearnObviousFlagsDirectly`
ModHooks seam (`packages/core/src/mod/hooks.ts`): a shape's obvious flags are
learned directly as runes when the mod's rule is on, matching the same set
`shapeLearnOnAssume` already computed rather than a broader "learn everything
the shape has" shortcut. The other three are not yet built.
