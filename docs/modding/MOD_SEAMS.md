...
- **`chained`** (`messageText`, `projectionRadius`, `historyDisplay`) runs in load order with each contributor seeing the previous one's output, a `reduce` over the contributors. Two mods narrowing one blast for two unrelated reasons both get their narrowing, and the last one still speaks last.
