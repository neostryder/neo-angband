...
| `registry:*` capabilities with real, wired, tested code | **15** (`blow`, `command`, `effect`, `effect-info`, `glyph`, `menu`, `monster`, `profile`, `projection`, `randart`, `room`, `rune`, `store`, `tval`, `vocab`), and the list is `REGISTRY_CAPABILITIES` in `mod/registry-host.ts`, which is the one source the vocabulary has |
