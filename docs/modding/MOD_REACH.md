...
| 9 | **UI is moddable** | **IN PROGRESS — phase 1 complete 2026-08-10** | The old evidence was stale: the command table is module-level and cached, not inside a keydown closure. Phase 1 extracts `GridSurface` from `GlyphTerm` (`packages/web/src/term.ts`) and makes tiles `RenderAssetRef` data, so a conforming grid renderer needs no Canvas2D context. Pointer input, client-coordinate hit testing, logical readback, and post-repaint size events are explicit capabilities. This is not a claim that the UI is fully moddable or that a 3D front end ships: menu/input/world data and the plugin-selected front-end seam remain the later phases of this row. |
