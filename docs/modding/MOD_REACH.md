...
reachable FROM DISK, so the non-bundled figure is 14/14 of the reachable seams
...
`packages/core/src/mod/registry-host.ts` is not a design note. All twelve facades
...
- The grammar is real and strict:
  `REGISTRY_RE = /^registry:(\*|effect-info|effect|room|profile|blow|store|command|monster|projection|glyph|randart|vocab)$/`
