...
| A mod can ADD a field core has never heard of, and read it at runtime | **YES** (2026-08-08) — `ObjectKind.ext` / `MonsterRace.ext`; was silently dropped by every binder before |
...
| 16 | **A mod can EXTEND a record, not only retune it** | **objects and monsters 2026-08-08; the other binders remain** | Measured on request: of the three operations an author expects - patch a value, remove a key, add a key - the first two worked end to end (a dagger patched to `1d5` really rolls it; dropping `flags` really removes THROWING) and the third did not. A new key composed cleanly, reported no problem, landed in the composed record, and was then dropped by the binder, so an author got no error and no effect. `ObjectKind.ext` and `MonsterRace.ext` now carry the keys core does not bind, frozen, absent entirely when a mod added none. Which keys are core's is DERIVED from core's own gamedata (`mod/record-keys.ts`, generated) rather than hand-listed, and its test re-derives it from the pack in both directions - a superset silently swallows a mod's field, a subset hands every mod an `ext` full of core's own. Core never reads `ext`: it is the data half of extending the game, and the behaviour half is `registry:effect` / `registry:blow`. Proven from disk. REMAINING: the other binders (ego items, artifacts, terrain, traps, vaults, player classes and races, and the rest of `CoreRegistries`) still drop an added key, so this row is not closed. |
