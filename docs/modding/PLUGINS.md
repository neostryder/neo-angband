...
| `backupFolder` | `{ name(), choose(), forget(), write(name, text), onSave(fn), list() }`, a player-chosen folder for automatic save backups. Present only when your manifest declared `backup:folder` and the platform can actually offer a folder picker; absent everywhere else, so test for it rather than assuming it. `list()` (#24) answers every `.neochar` file currently in the folder, cheaply identified by lineage and character name/level - never a full decode, never an import |
...
| `wizard` | the game's whole debug set - depth, experience, gold, stats, acquirement, mapping, plus a catalogue of every item, creature and artifact this game has and which pack added each. Present only when your manifest declared `debug:wizard` and there is a game to drive. **Every method refuses until you call `sandbox()`**, which cuts the session loose from its save slot and cannot be undone: the character on disk keeps whatever the last save left, and nothing after that is written. Call `attached()` first to get the name for the question you put to the player. See `MOD_SEAMS.md` section 4f |
| `installMod` | `(bytes) => Promise<{ok, id, version, lines} \| {ok, problem, lines}>`, installing a **content** mod from the bytes of an archive. Present only when your manifest declared `mod:install`. Code is refused, and what you install lands **switched off** - the player enables it themselves, and a mod takes effect on reload. `lines` is the wording the Mods screen prints for the same outcome, per-requirement rows and all; print it rather than writing a second vocabulary for one concept |
| `reloadGame` | `() => Promise<void>`, the game's own mod-change reload: every plugin's `uninstall()`, the autoplayer's keyboard handed back, the live character written down, and the session resumed on that character. Present on exactly the terms `installMod` is, because installing something the process will never load is half an act. Not a permission - a plugin reaches `location` regardless - but `location.reload()` skips all four steps, and the third is the player's progress |
