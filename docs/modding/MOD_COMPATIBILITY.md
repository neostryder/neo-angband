...
| unreleased (2026-09-02) | API-2 Worker ABI | `modApi: 2` plus `runtime: "worker"` and a mod-relative `workerEntry` selects a dedicated module Worker that is booted by host code, rather than importing `plugin.js` into the renderer. Its independently versioned 1.1 protocol supplies immutable init data, logs, asynchronous preferences, own-asset bytes, bag migration, semantic commands, render-coalesced state snapshots, `ModDisplaySnapshot` display snapshots, and small host-rendered declarative panels. API-1 remains the separate trusted in-process ABI. |
