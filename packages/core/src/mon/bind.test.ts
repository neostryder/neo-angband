...
/**
 * THE SAME RULE THE STORE AND EGO BINDERS HAVE, on the four monster-domain
 * fields that resolve a name from a mod-appendable list: `base:` and
 * `friends-base:` name a MonsterBase (monster_base.json), `friends:` and
 * `shape:` name another MonsterRace (monster.json itself). `append` lets one
 * mod add to any of these, so "mod A gives a monster a base/friend/shape mod B
 * defines, the player disables mod B" is reachable the same ordinary way it
 * is for a store's stock line, and used to throw out of `bindCore` for the
 * whole game.
 */
describe("a monster reference a mod got wrong", () => {
  const BROKEN_BASE: MonsterRecordJson = {
    name: "ghostly gremlin",
    base: "no-such-base",
    color: "w",
  };

  it("throws when nothing touched the record (base)", () => {
    const pack = loadPack();
    expect(() =>
      bindMonsters({ ...pack, monsters: [...pack.monsters, BROKEN_BASE] }),
    ).toThrow(/mon: race ghostly gremlin: invalid base no-such-base/);
  });

  it("drops the whole race and names the mod when base does not resolve", () => {
    const pack = loadPack();
    const rec = {
      ...BROKEN_BASE,
      [PROVENANCE_KEY]: { owner: "mod-a" },
    } as MonsterRecordJson;
    const modded = bindMonsters({ ...pack, monsters: [...pack.monsters, rec] });

    /* THE RACE IS GONE, and nothing else is. */
    expect(modded.races).toHaveLength(pack.monsters.length);
    expect(modded.raceByName("ghostly gremlin")).toBeNull();
    expect(modded.refused).toEqual([
      expect.objectContaining({
        file: "monster",
        record: "ghostly gremlin",
        field: "base",
        id: "mod-a",
        why: expect.stringContaining("unknown base no-such-base"),
      }),
    ]);
  });

  it("throws when nothing touched the record (friends-base)", () => {
    const pack = loadPack();
    const rec: MonsterRecordJson = {
      name: "ghostly ally",
      base: "ghost",
      color: "w",
      "friends-base": [{ chance: 50, number: "1", name: "no such base" }],
    };
    expect(() =>
      bindMonsters({ ...pack, monsters: [...pack.monsters, rec] }),
    ).toThrow(/mon: race ghostly ally: invalid friends base no such base/);
  });

  it("drops a bad friends-base entry rather than throwing", () => {
    const pack = loadPack();
    const rec = {
      name: "ghostly ally",
      base: "ghost",
    ...
    const race = modded.raceByName("ghostly ally");
    expect(race, "the race was dropped instead of the entry").not.toBeNull();
    expect(race?.friendsBase).toEqual([]);
    expect(modded.refused).toEqual([
      expect.objectContaining({
        file: "monster",
        record: "ghostly ally",
  ...
  it("throws when nothing touched the record (friends)", () => {
    const pack = loadPack();
    const rec: MonsterRecordJson = {
      name: "ghostly ally",
      base: "ghost",
      color: "w",
      friends: [{ chance: 50, number: "1", name: "no such friend" }],
    };
    expect(() =>
      bindMonsters({ ...pack, monsters: [...pack.monsters, rec] }),
    ).toThrow(/mon: could not find friend named 'no such friend' for 'ghostly ally'/);
  });

  it("drops a bad friend entry rather than throwing", () => {
    const pack = loadPack();
    const rec = {
      name: "ghostly ally",
      base: "ghost",
    ...
    expect(modded.refused).toEqual([
      expect.objectContaining({
        ...
        field: "friends",
        id: "mod-a",
        why: expect.stringContaining("could not find friend named 'no such friend'"),
      }),
    ]);
  });

  it("throws when nothing touched the record (shape)", () => {
    const pack = loadPack();
    const rec: MonsterRecordJson = {
      name: "ghostly shifter",
      base: "ghost",
      color: "w",
      shape: ["no such shape"],
    };
    expect(() =>
      bindMonsters({ ...pack, monsters: [...pack.monsters, rec] }),
    ).toThrow(/mon: could not find shape named 'no such shape' for 'ghostly shifter'/);
  });

  it("drops a bad shape entry rather than throwing", () => {
    const pack = loadPack();
    const rec = {
      name: "ghostly shifter",
      base: "ghost",
    ...
    const race = modded.raceByName("ghostly shifter");
    expect(race, "the race was dropped instead of the entry").not.toBeNull();
    expect(race?.shapes).toEqual([]);
    expect(modded.refused).toEqual([
      expect.objectContaining({
        file: "monster",
        record: "ghostly shifter",
        field: "shape",
        id: "mod-a",
        why: expect.stringContaining("could not find shape named 'no such shape'"),
      }),
    ]);
  });
