...
describe("inventory verbs (obj-gear.c)", () => {
  ...
  /**
   * packOverflowVictim (mod/hooks.ts seam, docs/modding/BUG_FIXES.md entry 11,
   * neostryder/neo-angband#116). Same overflow fixture as the catch-all test
   * above - sword sorts last in gear.inven, so the naive `handle === 0`
   * fallback always picks it - but here `firstPotion` is the item the caller
   * says just left the quiver. Faithful core (no hook, or previousQuiver with
   * no hook) still sheds the sword; an installed hook can redirect the shed to
   * whatever departedQuiver names instead.
   */
  describe("packOverflow's NULL-victim path consults packOverflowVictim", () => {
    function overflowFixture(): { state: ReturnType<typeof makeState>; sword: number; firstPotion: number } {
      const state = makeState({ playerGrid: loc(5, 5), commands: [] });
      const sword = carry(state, makeNamed("& Dagger~", TV.SWORD));
      let firstPotion = 0;
      for (let i = 0; i < constants.packSize; i++) {
        ...
        if (i === 0) firstPotion = h;
      }
      expect(packIsOverfull(state.gear, constants)).toBe(true);
      calcInventory(state.gear, constants);
      return { state, sword, firstPotion };
    }

    it("faithful core (no hook) sheds the naive trailing item even with previousQuiver supplied", () => {
      const { state, sword, firstPotion } = overflowFixture();
      packOverflow(state, 0, constants, { previousQuiver: [firstPotion] });
      expect(gearGet(state.gear, sword)).toBeNull();
      expect(gearGet(state.gear, firstPotion)).not.toBeNull();
    });

    it("an installed hook redirects the shed to the item departedQuiver names", () => {
      const { state, sword, firstPotion } = overflowFixture();
      let seenDeparted: number | null | undefined;
      packOverflow(state, 0, constants, {
        previousQuiver: [firstPotion],
        hooks: {
          packOverflowVictim: (_s, departed) => {
            seenDeparted = departed;
            return departed;
          },
        },
      });
      expect(seenDeparted).toBe(firstPotion);
      expect(gearGet(state.gear, firstPotion)).toBeNull();
      expect(gearGet(state.gear, sword)).not.toBeNull();
    });

    it("departedQuiver is null when nothing in previousQuiver actually left the quiver", () => {
      const { state, sword, firstPotion } = overflowFixture();
      let seenDeparted: number | null | undefined;
      packOverflow(state, 0, constants, {
        /* firstPotion is still in state.gear (never was in the quiver to begin
         * with, and calcInventory never put it there) - nothing "departed". */
        previousQuiver: [],
        hooks: {
          packOverflowVictim: (_s, departed) => {
            seenDeparted = departed;
      ...
      expect(seenDeparted).toBeNull();
      /* The hook declined (returned the null it was handed), so core falls
       * back to the faithful trailing-item selection. */
      expect(gearGet(state.gear, sword)).toBeNull();
      expect(gearGet(state.gear, firstPotion)).not.toBeNull();
    });
  });
