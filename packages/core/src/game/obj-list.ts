...
export function objectListStandardCompare(
  ...
  /* The comparator's last resort, for two entries that compared equal on every
   * upstream key INCLUDING distance. Faithful 4.2.6 leaves them equal, which
   * feeds a stable Array.sort and so keeps collect order - upstream's own
   * compare_items (obj-util.c) is not a strict weak order and can come out
   * unstable, and that wart is core's to keep. A mod may impose a further
   * ordering through the objectListTiebreak hook (mod/hooks.ts); absent one, the
   * `?? 0` below IS the faithful answer. */
  const tiebreak = state.modHooks?.objectListTiebreak;
