...
export function newGear(): Gear {
	return {
		store: new Map<number, GameObject>(),
		next: 1,
		pack: [],
		inven: [],
		quiver: []
 ...
 */
export function preferredQuiverSlot(
	obj: GameObject,
	rogueLike = false
): number {
	let desiredSlot = -1;
	if (obj.note && (tvalIsAmmo(obj.tval) || obj.flags.has(OF.THROWING))) {
		...
		while (s !== -1) {
			...
			if (k === fireKey || k === throwKey) {
				...
				desiredSlot =
					(obj.note.charCodeAt(s + 2) || 0) - '0'.charCodeAt(0);
				break;
			}
...
export interface MinusAcEnv {
	...
	/**
	 * object_desc(ODESC_BASE): the bare armour name.
	 *
	 * REQUIRED, and required the hard way - it used to default to
	 * `obj.kind.name`, which is the raw object.txt template, not a name. The
	 * live projection path (session/game.ts, acid traps and acid balls) never
	 * supplied this, so a player hit by acid read
	 * `Your & Leather Shield~ is damaged!` - the article and plural markers
	 * object_desc exists to resolve. The venom-sting path did supply it, so the
	 * same message was right or wrong depending on where the acid came from.
	 *
	 * A fallback that is silently wrong at every call site that forgets is worse
	 * than no fallback: nothing fails, and the defect only surfaces in play. A
	 * missing supplier is now a compile error.
	 */
...
export function invenCarryResult(
	...
	for (const handle of gear.pack) {
		...
		const mode = objectIsInQuiver(gear, handle)
			? OSTACK_QUIVER
			: OSTACK_PACK;
	...
	for (const handle of gear.pack) {
		...
		if (tvalIsAmmo(obj.tval) || obj.flags.has(OF.THROWING)) {
			if (objectIsInQuiver(gear, handle)) {
				quiverAmmo +=
					obj.number *
					(tvalIsAmmo(obj.tval) ? 1 : constants.thrownQuiverMult);
				found = true;
			}
		}
		/* Count regular slots. */
...
export function quiverAbsorbNum(
	...
	if (ammo || obj.flags.has(OF.THROWING)) {
		...
		for (let i = 0; i < qSize; i++) {
			...
			const quiverObj =
				qHandle !== 0 ? gear.store.get(qHandle) : undefined;
			if (quiverObj) {
				const mult = tvalIsAmmo(quiverObj.tval)
					? 1
					: constants.thrownQuiverMult;
		...
		if (spaceFree && ((displaces && nEmpty) || !displaces)) {
			...
			const nToQuiver = Math.min(
				obj.number,
				Math.trunc(spaceFree / mult)
			);
			const usedPack =
				nAddPack -
...
export function invenCarryNum(
	...
	const { nToQuiver, nAddPack } = quiverAbsorbNum(
		gear,
		obj,
		constants,
		nFreeSlot
...
export function calcInventory(
	...
	for (const handle of [...gear.pack]) {
		...
		if (
			...
			const mult = tvalIsAmmo(current.tval)
				? 1
				: constants.thrownQuiverMult;
			let toQuiver = false;
...
function invenCanStackPartial(
	...
	if (mode1 & OSTACK_QUIVER) {
		...
		if (mode2 & ~OSTACK_QUIVER) {
			const nFreeSlot =
				constants.packSize - packSlotsUsed(gear, constants);
			const { nToQuiver } = quiverAbsorbNum(
				gear,
				obj2,
				constants,
				nFreeSlot
...
export function combinePack(
	...
	for (const h1 of [...gear.pack].reverse()) {
		...
		for (let k = 0; k < idx1; k++) {
			...
			if (!obj2) continue;
			const mode2 = objectIsInQuiver(gear, h2)
				? OSTACK_QUIVER
				: OSTACK_PACK;
			...
			const mode1 = objectIsInQuiver(gear, h1)
				? OSTACK_QUIVER
				: OSTACK_PACK;
			if (
				invenCanStackPartial(gear, obj2, obj1, mode2, mode1, constants)
			) {
...
export function packTotalSuppressed(obj: GameObject): boolean {
	return (
		tvalCanHaveCharges(obj.tval) || tvalIsRod(obj.tval) || obj.timeout > 0
	);
}
...
export function gearObjectForUse(
	...
	const obj = gear.store.get(handle);
	if (!obj)
		throw new Error(`gearObjectForUse: no object for handle ${handle}`);
...
export function outfitPlayer(
	...
	for (const si of player.cls.startItems) {
		...
		if (sval < 0)
			throw new Error(`outfit: unknown sval ${si.tval}:${si.sval}`);
