...
function buildPack(inject: boolean): CorePack {
  return {
    ...
    obj: {
      ...
      brand: { ...loadJson<object>("brand"), records: records("brand", inject) },
      slay: { ...loadJson<object>("slay"), records: records("slay", inject) },
...
const COVERED_FILES: Record<string, (p: Bound) => readonly (ModExtensible | null)[]> = {
  ...
  brand: (p) => p.reg.objects.brands,
  slay: (p) => p.reg.objects.slays,
