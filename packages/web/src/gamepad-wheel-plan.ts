 ...
 * `docs/design/GAMEPAD_CONTROLS.md`.
 *
 * A command is matched by its ORIGINAL-keyset key where it has one, and by its
 * label where it does not - the eleven root commands that live outside the
 * cached keypress registry carry no key at all. Keys are one character or a
 * caret pair, labels are words, so the two cannot collide.
 */
import type { ControlAction, ControlCommand } from "./control-surface";
...
/** How a command is looked up: its original-keyset key, else its label. */
export function selectorFor(command: { readonly key?: string; readonly label: string }): string {
...
export function entryFor(command: ControlCommand): WheelEntry {
  const named = ENTRIES[selectorFor(command)];
...
export function groupCommands(
  ...
  if (group.members.length === 0) {
    return commands.filter((command) => !CLAIMED.has(selectorFor(command)));
  }
  const bySelector = new Map(commands.map((command) => [selectorFor(command), command]));
