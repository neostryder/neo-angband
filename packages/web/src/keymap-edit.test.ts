...
describe("runKeymapEditor (do_cmd_keymaps)", () => {
  ...
  // #62: F1-F12 could not be captured as a keymap TRIGGER at all.
  it("accepts a plain F-key as a create trigger (#62)", async () => {
    const term = makeTerm();
    const done = runKeymapEditor(term, false);
    await tick();
    press(win, "b"); // Create
    await tick();
    press(win, "F5"); // trigger
    await tick();
    press(win, "q");
    press(win, "=");
    await tick();
    press(win, "y"); // confirm
    await tick();
    expect(keymapFind("orig", "F5")).toBe("q");
    press(win, "Escape"); // ack
    await tick();
    press(win, "Escape"); // leave
    await done;
  });

  // #63: Enter was rejected as a keymap TRIGGER, blocking a keymap keyed on
  // Enter itself (as distinct from Enter appearing inside an action - the
  // next test).
  it("accepts Enter as a create trigger (#63)", async () => {
    const term = makeTerm();
    const done = runKeymapEditor(term, false);
    await tick();
    press(win, "b"); // Create
    await tick();
    press(win, "Enter"); // trigger
    await tick();
    press(win, "q");
    press(win, "=");
    await tick();
    press(win, "y"); // confirm
    await tick();
    expect(keymapFind("orig", "Enter")).toBe("q");
    press(win, "Escape"); // ack
    await tick();
    press(win, "Escape"); // leave
    await done;
  ...
  // #63's own example: rest-until-reason, "R&" then Enter to confirm. Enter
  // used to be silently swallowed while capturing an ACTION; it must now
  // both be accepted and be recoverable as its own keypress on replay, not
  // spliced into "R&Enter" as five stray letters.
  it("stores Enter within an action sequence as a single token (#63)", async () => {
    const term = makeTerm();
    const done = runKeymapEditor(term, false);
    await tick();
    press(win, "b"); // Create
    ...
    press(win, "R");
    press(win, "&");
    press(win, "Enter"); // part of the action, not yet '=' to finish
    await tick();
    expect(term.snapshot()[0]).toContain("R&[Enter]");
    press(win, "="); // finish
    await tick();
    press(win, "y"); // confirm
    await tick();
  ...
  it("Backspace during action capture removes one whole Enter token, not one letter", async () => {
    const term = makeTerm();
    const done = runKeymapEditor(term, false);
    await tick();
    press(win, "b"); // Create
    ...
    press(win, "R");
    press(win, "Enter");
    press(win, "Backspace"); // removes the whole "[Enter]" token
    press(win, "=");
    await tick();
    press(win, "y"); // confirm
    await tick();
    expect(keymapFind("orig", "X")).toBe("R");
    press(win, "Escape"); // ack
    await tick();
    press(win, "Escape"); // leave
    await done;
