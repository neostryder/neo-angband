...
import type { ControlCommand } from "./control-surface";
...
import {
  ...
  namedSelectors, replyEntry, selectorFor, shortWord, unreachableCommands,
...
describe("the command wheel's split", () => {
  ...
  it("names a word and an icon for every shipped command", () => {
    const unnamed = commands
      .filter((command) => entryForSelector(selectorFor(command)) === undefined)
