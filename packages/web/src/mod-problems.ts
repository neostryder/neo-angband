...
/**
 * The same problem said once.
 *
 * Needed because ONE rule can be enforced by more than one reader, and should be:
 * the engine gate (mod-engine.ts) runs on the content path, the code path and the
 * tiles path, so a mod that ships records AND a plugin fails it twice and each
 * loader reports it honestly. The alternative - nominating one path to report and
 * asking the others to stay quiet - is a rule that silently stops working the day
 * the nominated path is not the one that ran.
 *
 * Identity is (id, why): two different faults on one mod are two problems, and the
 * same sentence about the same mod is one, however many readers reached it.
 */
export function dedupeProblems(problems: readonly ModProblem[]): readonly ModProblem[] {
  const seen = new Set<string>();
  const out: ModProblem[] = [];
  for (const p of problems) {
    /* NUL as the separator, because it cannot occur in a mod id or a message.
     * Written as the ESCAPE and not as a literal NUL byte: this file used to
     * carry two real 0x00 bytes, which made every tool that sniffs binary -
     * ripgrep and git grep included - skip it entirely. A source file that no
     * content search can see is a source file nobody will find. */
    const key = `${p.id ?? "\0"}\0${p.why}`;
