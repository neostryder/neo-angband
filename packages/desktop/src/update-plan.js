...
export const PRESERVE = ["neo-angband-data"];
 ...
 */
export function updatability(shape) {
    if (!shape.packaged)
        return "none";
    if (shape.portableExecutableDir || shape.appImage)
        return "manual";
    if (!shape.writable)
        return "manual";
 ...
 */
export function installRoot(platform, execPath) {
    const parts = execPath.split(/[\\/]/u);
    const up = (n) => parts.slice(0, parts.length - n).join(platform === "win32" ? "\\" : "/");
    if (platform === "darwin") {
 ...
 */
export function swapScript(plan, pid, platform) {
    if (platform === "win32") {
        const q = psQuote;
