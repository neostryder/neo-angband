...
## CORRECTION 2026-07-25: S-3 was my error, not a finding

**The species-mix G-test was invalid, and I should have caught it before reporting
p-values of 1e-98.**

The bisect of the S-3 fix batch showed per-depth G swinging by ±200 in *both*
directions for changes that perturb the RNG stream, with no consistent sign. That
is the signature of a noisy metric, not of wrong changes. So I measured the noise
floor the way I should have from the start — the port against **itself** at a
second base seed, 400 levels per depth each:

| depth | df | G(port vs port, different seed) | G(port vs C) |
|---:|---:|---:|---:|
| 5 | 133 | 704.0 | 823.4 |
| 6 | 137 | 465.9 | 746.9 |
| 8 | 166 | 528.1 | 534.7 |
...
The port is the same distance from *itself* as from the C — and at depth 13 it is
**further from itself** than from the C. So "G = 389–929 at depths 5–20" was never
evidence of divergence.

### Why

**Clustering.** Monster placement is not independent per monster: a pit or nest
drops 20–60 monsters of one theme into a single level. The effective sample size
per depth is the number of **levels** (400), not the number of **monsters**
(~20 000). A G-test assuming one independent observation per monster inflates the
statistic by roughly the cluster size — exactly the 3–5× observed.

This is the *same* overdispersion I correctly identified for density: measured
per-level sd 21.6 where Poisson says 6.6, which is why the mean test uses a
measured sd and why I criticised the oracle stream's Poisson standard error. I
then applied a textbook independent-counts test to clustered count data one
function away. Same error, mine.

### What this changes

- **S-3 is withdrawn.** There is no established species-mix divergence.
- **The rejection of the S-3 fix batch stands as "not validated", not as
  "harmful".** RC1, RC3, the friends reversal and the room/vault reversal are all
  still justified by the C; the measurement simply could not arbitrate them. They
  need re-judging on C-derivation alone, or on a valid test.
- **The species assertion is un-gated** in `parity-c-stat.test.ts`, still measured
  and printed. Leaving a knowingly-invalid assertion red is precisely why I
  deleted `parity-c.test.ts`.

### What survives, with valid tests behind it

- **Density**: green at all 20 depths, pooled Stouffer Z = −1.22, p = 0.22, using
  a measured per-level sd. S-2 remains correctly closed.
- **Level feelings**: the G-test *is* valid here — each level contributes exactly
  one feeling sample, so observations are independent. And it now shows the real
  finding, far narrower than S-3 ever was:

  **Object level feeling diverges at depths 11, 12 and 16** (G = 30.7–40.4 on
  df = 7; p = 6.9e-5, 3.1e-5, 1.1e-6). Object feeling derives from the level's
  object rating, so this points at object generation value or quantity — not at
  monster selection.
- The unique-retirement harness fix stands on its own mechanism
  (`kill_all_monsters` zeroes `max_num`), verified against the C rather than by G.

### To make a species test valid

The level must be the unit of observation — a permutation null over per-level
species vectors. That is blocked on the C side: `main-stats`' schema stores
per-depth aggregates only (`monsters(level, count, k_idx)`, summed across runs), so
per-run C samples do not exist. Emitting them requires a change to the DB writer
in the oracle **build copy** (never in `reference/`). That is the next step, and
until it lands no species claim should be made in either direction.
