...
const DECL =
  /(?:function|const|let|var|class|interface|type|enum)\s+([A-Za-z_$][\w$]*)|^[ \t]*(?:readonly\s+|public\s+|private\s+|static\s+|\*\s*)?([A-Za-z_$][\w$]*)\s*[(<]/gm;
const index = new Map();
...
for (const it of items) {
  ...
  const namedCite = files.some((f) => !f.isTest && f.src.includes(it.name));
  const base = it.file.split("/").pop();
  const areaWorked = files.some((f) => !f.isTest && f.src.includes(base));

  let verdict;
  if (hit && namedCite) verdict = "PORTED-AND-CITED";
  ...
  out.push({ ...it, verdict, via, where: (hit?.[0] ?? "").replace("packages/", "") });
}
