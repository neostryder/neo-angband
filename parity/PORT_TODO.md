...
- [ ] **0.1 Adjudicate the ledger `deferred:` items. 335 of 339 done, 72 of the
  ...
  **`obj-ignore` and `project-path` next, and `real` went UP** — 6 to 9, the
  first time in this whole pass. That is the sweep working, not failing: three
  rows that had been excusing themselves turned out to name work that is
  genuinely owed, and now say so.

  - **A decoy does not stop a bolt.** Decoys are modelled — `GameState.decoy` is
    written, read and persisted — but `world/project.ts:50` still holds
...
- [ ] **5.5 `randart.log` / `randart.txt`. IN PROGRESS — the file exists and
  186 of its 233 emission sites are written. The remainder is MEASURED, not
  estimated.**
  ...
  **The row's own count was low.** "193 `file_putf` sites" is the raw grep of
  ...
  | `obj-randart.c` `file_putf(log_file, …)` — the design loop | 174 | 127 |
  ...
  **How the remainder is kept visible.** There is no compiled Angband here and no
  ...
  because the FIRST attempt at that check silently failed to apply its own edit,
  and a mutation that never lands looks exactly like one that survives.
